...
metadata:
  name: gpu-nvidia-a100
  annotations:
    ome.ai/kueue-resource-flavor: "gpu-nvidia-a100"
spec:
...
func (s *RuntimeSelector) buildEffectiveRequirements(
    ...
    if class, ok := annotations["ome.ai/accelerator-class"]; ok {
        req.PreferredClasses = []string{class}
    }
    if strategy, ok := annotations["ome.ai/selection-strategy"]; ok {
