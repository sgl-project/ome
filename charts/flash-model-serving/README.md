...
- `templates/supported_models_configmap.yaml` publishes a curated Flash catalog in the `ome` namespace. The control plane reads `supported-model-catalog.yaml` to find exact external Hugging Face model IDs and the `ClusterServingRuntime` resources that can serve them.
