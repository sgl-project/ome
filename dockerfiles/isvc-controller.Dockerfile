# Build the isvc-controller binary
...
# Build the isvc-controller binary (CGO disabled for static binary)
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    -o isvc-controller ./cmd/isvc

# Use scratch for minimal image
FROM scratch
WORKDIR /
COPY --from=builder /workspace/isvc-controller .
USER 65532:65532

ENTRYPOINT ["/isvc-controller"]
