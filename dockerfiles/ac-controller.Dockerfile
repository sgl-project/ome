# Build the ac-controller binary
...
# Build the ac-controller binary (CGO disabled for static binary)
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    -o ac-controller ./cmd/ac

# Use scratch for minimal image
FROM scratch
WORKDIR /
COPY --from=builder /workspace/ac-controller .
USER 65532:65532

ENTRYPOINT ["/ac-controller"]
