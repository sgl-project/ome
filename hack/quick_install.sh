...
# Install ome
OME_CONFIG=ome.yaml
MAJOR_VERSION=$(echo ${OME_VERSION:1} | cut -d "." -f1)
MINOR_VERSION=$(echo ${OME_VERSION} | cut -d "." -f2)
...
kubectl apply -f https://bitbucket.oci.oraclecorp.com/genaicore/ome/releases/download/${OME_VERSION}/${OME_CONFIG}
...
if [ ${MAJOR_VERSION} -eq 0 ] && [ ${MINOR_VERSION} -le 11 ]; then
    kubectl apply -f https://bitbucket.oci.oraclecorp.com/genaicore/ome/releases/download/${OME_VERSION}/ome-runtimes.yaml
else
    kubectl apply -f https://bitbucket.oci.oraclecorp.com/genaicore/ome/releases/download/${OME_VERSION}/ome-cluster-resources.yaml
fi
...
EOF
kubectl patch cm inferenceservice-config -n ome --type=merge --patch-file=deploy-config-patch.yaml
fi
echo "😀 Successfully installed OME"
