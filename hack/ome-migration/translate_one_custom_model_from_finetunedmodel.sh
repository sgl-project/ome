...
fetch_finetunedmodel_yaml() {
  local name=$1
  local out_file=$2

  log "Fetching FineTunedModel YAML for $name"
  kubectl get finetunedmodel "$name" -o yaml > "$out_file"
}

convert_finetunedmodel_to_finetunedweight() {
  local input_yaml=$1
  local output_yaml=$2

  log "Converting FineTunedModel CR to FineTunedWeight CR"

  local json=$(yq eval -o=json "$input_yaml")

  # Extract values from FineTunedModel CR
  ...
  local base_model_ref=$(jq -r '.spec.baseModelRef.name' <<< "$json")
  local storage_url=$(jq -r '.spec.storageUrl' <<< "$json")
  local vendor="cohere"

  # Extract hyperparameters and strategy
  declare -A hyperparameters
  local strategy=""
  for param in $(jq -r '.spec.hyperparameters[] | @base64' <<< "$json"); do
...
patch_finetunedweight_status() {
  ...
  if [[ -n "$model_state" ]]; then
    ...
    log "Successfully patched the FineTunedWeight $model_name"
  else
    log "FineTunedModel status.state is empty, skipping status patch"
  fi
}

main() {
  if [[ $# -ne 1 ]]; then
    error "Usage: $0 <finetunedmodel-name>"
  fi
  ...
  validate_dependencies
  fetch_finetunedmodel_yaml "$name" "$original_yaml"
  convert_finetunedmodel_to_finetunedweight "$original_yaml" "$converted_yaml"
  apply_finetunedweight "$converted_yaml"
  log "FineTunedModel to FineTunedWeight conversion complete for $name"
