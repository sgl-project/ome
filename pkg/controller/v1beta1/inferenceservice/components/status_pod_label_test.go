...
func TestRawDeploymentPodLabelInfoUsesTruncatedComponentName(t *testing.T) {
	...
	statusSpec := v1beta1.ComponentStatusSpec{LatestReadyRevision: "revision-name"}
...
func TestNonRawPodLabelInfoUsesRevision(t *testing.T) {
	...
	if value != statusSpec.LatestReadyRevision {
		t.Fatalf("label value = %q, want %q", value, statusSpec.LatestReadyRevision)
	}
}
