...
func TestInferenceServiceReconcile(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "External service URL setting when ingress disabled",
			isvc: &v1beta1.InferenceService{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-external-service-url",
					Namespace: "default",
				},
				Spec: v1beta1.InferenceServiceSpec{
					Model: &v1beta1.ModelRef{
			...
			setupMocks: func(c client.Client, cs *fake.Clientset) {
				// Create inferenceservice config with ingress disabled
				cm := &v1.ConfigMap{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "inferenceservice-config",
						Namespace: "default",
					...
					Data: map[string]string{
						"config": `{"ingressConfig":{"disableIngressCreation":true}}`,
					},
				}
				err := c.Create(context.TODO(), cm)
				g.Expect(err).NotTo(gomega.HaveOccurred())
			...
			validate: func(t *testing.T, c client.Client, isvc *v1beta1.InferenceService) {
				// Check that external service was created
				...
				g.Expect(err).NotTo(gomega.HaveOccurred())
				g.Expect(externalSvc.Spec.Selector).To(gomega.Equal(map[string]string{
					"app":                          "test-external-service-url-engine",
					"component":                   "engine",
					"app.kubernetes.io/name":      "inference-service",
					"app.kubernetes.io/instance":  "test-external-service-url",
					"app.kubernetes.io/component": "engine",
				}))

				// Get the updated InferenceService to check status
				...
				// Verify that Status.Address and Status.URL are set
				g.Expect(updatedIsvc.Status.Address).NotTo(gomega.BeNil())
				g.Expect(updatedIsvc.Status.Address.URL).NotTo(gomega.BeNil())
				g.Expect(updatedIsvc.Status.Address.URL.String()).To(gomega.Equal("http://test-external-service-url.default.svc.cluster.local"))
				g.Expect(updatedIsvc.Status.URL).NotTo(gomega.BeNil())
				g.Expect(updatedIsvc.Status.URL.String()).To(gomega.Equal("http://test-external-service-url.default.svc.cluster.local"))
				
				// Verify IngressReady condition is set to True with reason IngressDisabled
				ingressReadyCondition := updatedIsvc.Status.GetCondition(v1beta1.IngressReady)
				g.Expect(ingressReadyCondition).NotTo(gomega.BeNil())
				g.Expect(ingressReadyCondition.Status).To(gomega.Equal(v1.ConditionTrue))
				g.Expect(ingressReadyCondition.Reason).To(gomega.Equal("IngressDisabled"))
			},
		},
	}
