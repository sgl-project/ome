...
// JobReconciler reconciles the raw Kubernetes Job resource
...
func (jr *JobReconciler) Reconcile() (*batchv1.Job, error) {
	...
	log.Info("Training job reconcile", "checkResult", checkResult, "training_job", jr.Job.Name)

	switch checkResult {
	case constants.CheckResultCreate:
		err = jr.client.Create(context.TODO(), jr.Job)
		if err != nil {
			log.Info("Failed to create training job", "training_job", jr.Job.Name)
			return nil, err
		} else {
			return jr.Job, nil
		}
	case constants.CheckResultUpdate:
		...
		if err != nil {
			log.Info("Failed to update training job", "training_job", jr.Job.Name)
			return nil, err
		} else {
			return jr.Job, nil
		}
...
func (jr *JobReconciler) checkJobExist(client client.Client) (constants.CheckResultType, *batchv1.Job, error) {
	...
	if err != nil {
		if apierr.IsNotFound(err) {
			log.Info("K8s training job not found, will be created", "training_job", jr.Job.Name)
			return constants.CheckResultCreate, nil, nil
		}
		log.Error(err, "Failed to get training job", "training_job", jr.Job.Name)
