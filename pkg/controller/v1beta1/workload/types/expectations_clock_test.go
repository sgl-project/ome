...
func TestExpectations_TTLBoundary(t *testing.T) {
	...
	// Exactly at the deadline: expired. The deadline is the instant the
	// failsafe fires, not the last instant it is withheld.
	fc.SetTime(t0.Add(expectationsTTL))
	if !e.Satisfied("ns", "isvc", ComponentEngine, 0) {
		t.Fatal("an expectation at its exact deadline must be treated satisfied")
	}
}

func TestExpectations_ExpiresPastTTL(t *testing.T) {
	t0 := time.Date(2026, 7, 22, 12, 0, 0, 0, time.UTC)
	fc := clocktesting.NewFakeClock(t0)
	e := NewExpectationsWithClock(fc)
