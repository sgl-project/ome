...
type WorkloadDesiredSpec struct {
	...
	//
	// Inert: the adapter that projects the InferenceReplica spec fills
	// this in, but no rollout-engine path reads it. Availability comes
	// from EndpointSlice membership, not a readiness-age gate, so the
	// spec field it carries has no effect on rollout pacing, and the
	// AvailablePodCount doc in types.go ("ready for at least
	// MinReadySeconds") describes the intent rather than the behaviour.
	// Honoring it means teaching availability to consult it, not
	// changing what sets it.
...
//
// Inert: no rollout-engine path reads Partition, MaxUnavailable or
// Decisions. The adapter that projects the InferenceReplica spec fills
// Partition and MaxUnavailable in, but pacing actually flows through
// RollingUpdate.Partition and the UpdateGate callback, and availability
// comes from EndpointSlice membership — so the two spec fields they
// carry have no effect. Decisions is never constructed outside tests.
// The rollback-to-revision half of the same spec stanza is live; it
// takes a different path and does not pass through here.
