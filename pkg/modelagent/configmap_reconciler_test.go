...
func TestFindMatchedModelFromConfigMap_FindsMatch(t *testing.T) {
	...
	modelKey, parent, raw, err := reconciler.FindMatchedModelFromConfigMap(cm, "abc123", "namespace.basemodel")

	assert.Equal(t, "namespace.basemodel.model2", modelKey)
	assert.Equal(t, "/models/model2", parent)

	expectedJSON := buildJSONWith("abc123", "/models/model2")
...
func TestFindMatchedModelFromConfigMap_NotFound_NoError(t *testing.T) {
	...
	modelKey, parent, raw, err := reconciler.FindMatchedModelFromConfigMap(cm, "missing-sha", "clusterbasemodel")
...
func TestFindMatchedModelFromConfigMap_NotFound_WithInvalidJSON_ReturnsError(t *testing.T) {
	...
	modelKey, parent, raw, err := reconciler.FindMatchedModelFromConfigMap(cm, "nope", "namespace.basemodel")
	assert.Empty(t, modelKey)
	assert.Empty(t, parent)
	assert.Empty(t, raw)
	if err == nil {
...
func TestFindMatchedModelFromConfigMap_SkipsIncompleteEntries(t *testing.T) {
	...
	modelKey, parent, raw, err := reconciler.FindMatchedModelFromConfigMap(cm, "target-sha", "namespace.basemodel")
	assert.Empty(t, modelKey)
	assert.Empty(t, parent)
	assert.Empty(t, raw)
...
func TestUpdateConfigMapWithUpdatedChildrenPaths_Success(t *testing.T) {
	...
	// Perform update
	err = reconciler.updateConfigMapWithUpdatedChildrenPaths(ctx, key, "/b", existing)
	assert.NoError(t, err)
...
func TestUpdateConfigMapWithUpdatedChildrenPaths_MissingKey_NoOp(t *testing.T) {
	...
	cm := &corev1.ConfigMap{
		...
		Data: map[string]string{
			"some.other.key": existing,
		},
	}
	_, err := kubeClient.CoreV1().ConfigMaps(reconciler.namespace).Create(ctx, cm, metav1.CreateOptions{})
	assert.NoError(t, err)

	// Execute (should be no-op and no error)
	err = reconciler.updateConfigMapWithUpdatedChildrenPaths(ctx, key, "/b", existing)
	assert.NoError(t, err)
...
func TestUpdateConfigMapWithUpdatedChildrenPaths_InvalidExistingJSON(t *testing.T) {
	...
	err = reconciler.updateConfigMapWithUpdatedChildrenPaths(ctx, key, "/b", currentDataEntry)
