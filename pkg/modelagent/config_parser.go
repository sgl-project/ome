...
import (
	...
	"bitbucket.oci.oraclecorp.com/genaicore/ome/pkg/client/clientset/versioned"
...
func (p *ModelConfigParser) determineModelCapabilitiesFromHF(hfModel modelconfig.HuggingFaceModel) []string {
	...
	// For vision, only support image chat capability right now for project flash scope.
	// Image embed capability not supported yet in current project flash scope, and is handled differently in service-sourced model scope (Embed capability + additional metadata)
