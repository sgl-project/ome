...
func (s *Gopher) processHuggingFaceModel(ctx context.Context, task *GopherTask, baseModelSpec v1beta1.BaseModelSpec,
	...
	sha, err := hub.FetchAttributeFromHfModelMetaData(ctx, hfComponents.ModelID, hub.Sha)
	if err != nil {
		s.logger.Errorf("Failed to retrieve sha from Hugging Face for model %s: %s", hfComponents.ModelID, err)
	}
	shaStr := sha.(string)

	matchedModelTypeAndModeName, parentPath, modelDataEntry := s.handelReuseArtifactIfNecessary(ctx, baseModelSpec, modelType, hfComponents.ModelID, namespace, shaStr)
	if matchedModelTypeAndModeName != "" {
		...
		err = s.configMapReconciler.updateConfigMapWithUpdatedChildrenPaths(ctx, matchedModelTypeAndModeName, destPath, modelDataEntry)
		if err != nil {
			s.logger.Errorf("fail to update configmap to add new path to childrenPaths: %s", err)
			return err
		}
		s.logger.Infof("successfully create symbolic link and add the new path to childrenPath for model: %s", modelInfo)
		return nil
	} else {
...
// TODO: whether with best efforts to locate the existing path? is there any cm reconciliation method?
/*
...
func (s *Gopher) handelReuseArtifactIfNecessary(ctx context.Context, baseModelSpec v1beta1.BaseModelSpec,
	modelType string, modelId string, namespace string, shaStr string) (string, string, string) {
	// check whether identical artifact is already existing if model specified with ReuseIfExists
	if baseModelSpec.Storage.DownloadPolicy == v1beta1.ReuseIfExists {
		var matchedModelTypeAndModelName string
		var parentPath string
		var modelDataEntry string
		var err error
		...
		if modelType == "ClusterBaseModel" || modelType == "BaseModel" {
			matchedModelTypeAndModelName, parentPath, modelDataEntry, err = s.configMapReconciler.getModelDataByArtifactSha(ctx, shaStr, "clusterbasemodel")
			if err != nil {
				s.logger.Warnf("get error when finding matched model in configmap for model : %s: %s", modelId, err)
			}
		}
		if modelType == "BaseModel" && matchedModelTypeAndModelName == "" {
			// build namespaced model type
			namespacedModelType := fmt.Sprintf("%s.basemodel", namespace)
			matchedModelTypeAndModelName, parentPath, modelDataEntry, err = s.configMapReconciler.getModelDataByArtifactSha(ctx, shaStr, namespacedModelType)
			if err != nil {
				s.logger.Warnf("get error when finding matched model in configmap for model : %s: %s", modelId, err)
			}
		}
		return matchedModelTypeAndModelName, parentPath, modelDataEntry
	}
	return "", "", ""
}
