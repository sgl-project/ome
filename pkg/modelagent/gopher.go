...
const (
	...
	defaultStartupConfigMapSnapshotTimeout = 5 * time.Second
)
...
func (s *Gopher) Run(stopCh <-chan struct{}, numWorker int, numHighPriorityWorker int) {
	startupRecoveryCtx, cancelStartupRecovery := startupConfigMapContext()
	s.recoverStartupHuggingFaceArtifactParents(startupRecoveryCtx)
	cancelStartupRecovery()

	startupSnapshotCtx, cancelStartupSnapshot := startupConfigMapContext()
	s.captureStartupReadyModels(startupSnapshotCtx)
	cancelStartupSnapshot()
...
func (s *Gopher) runWorker() {
	...
	for {
		...
		if s.deferStartupReadyRevalidationIfLocalPathExists(task) {
			continue
		}
			...
			if useHuggingFaceOriginRepair {
				...
				if reused {
					...
					if !reused {
						...
						cacheConfig := s.modelArtifactCache.normalized()
						if cacheConfig.enabled() {
							var cacheSource string
							hydratedFromCache, cacheSource, err = s.tryReuseMountedArtifactCache(ctx, hfOriginIdentity, rebuildParentPath)
							if err != nil {
								s.metrics.RecordFailedDownload(modelType, namespace, name, "mounted_artifact_cache_error")
								if cacheConfig.SourceRequired {
									if reserved {
										_ = s.configMapReconciler.deleteConfigMapDataEntry(ctx, hfArtifactParentKey)
									} else {
										_ = s.configMapReconciler.markHuggingFaceArtifactParentFailed(ctx, hfArtifactParentKey, hfOriginIdentity, rebuildParentPath)
									}
									s.markModelOnNodeFailed(task)
									return err
								}
								s.logger.Warnf("Mounted model artifact cache cannot hydrate %s/%s: %v; falling back to Object Storage", namespace, name, err)
								err = nil
							}
							if hydratedFromCache {
								s.logger.Infof("Hydrated canonical Hugging Face artifact parent for %s/%s from mounted model artifact cache %s at %s", namespace, name, cacheSource, rebuildParentPath)
							}
						}
						if !hydratedFromCache {
							err = downloadObjectStorageModel(rebuildParentPath)
						}
						if err != nil {
							if reserved {
					...
					return nil
				} else if hasHFOriginIdentity && !useHuggingFaceOriginReuse {
					if !allowFallbackDownload {
						s.demoteToNormalPriority(task)
						return nil
					}
					hydratedFromCache, cacheSource, cacheErr := s.tryReuseMountedArtifactCache(ctx, hfOriginIdentity, destPath)
					if cacheErr != nil {
						s.metrics.RecordFailedDownload(modelType, namespace, name, "mounted_artifact_cache_error")
						if s.modelArtifactCache.normalized().SourceRequired {
							s.markModelOnNodeFailed(task)
							return cacheErr
						}
						s.logger.Warnf("Mounted model artifact cache cannot hydrate %s/%s: %v; falling back to Object Storage", namespace, name, cacheErr)
					}
					if hydratedFromCache {
						s.logger.Infof("Hydrated legacy OCI model %s/%s from mounted model artifact cache %s at %s", namespace, name, cacheSource, destPath)
...
func (s *Gopher) captureStartupReadyModels(ctx context.Context) {
	...
	s.startupHuggingFaceParentValidationMutex.Unlock()
	s.logger.Infof("Captured %d Ready models and %d Ready Hugging Face artifact parents from startup ConfigMap snapshot", len(readyModelKeys), len(readyHuggingFaceParentKeys))
}
