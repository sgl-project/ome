...
func TestLoadMiniCPMConfig(t *testing.T) {
	tests := []struct {
		name         string
		configPath   string
		expectedType string
		...
		{
			name:         "MiniCPM3 4B",
			configPath:   "testdata/minicpm3_4b.json",
			expectedType: "minicpm3",
			minParams:    3_500_000_000,
			maxParams:    4_500_000_000,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			config, err := LoadMiniCPMConfig(tt.configPath)
			if err != nil {
			...
			if config.HasVision() {
				t.Error("HasVision() should return false for base MiniCPM models")
			}
		})
	}
}

func TestMiniCPMConfigInterface(t *testing.T) {
	// Test that MiniCPMConfig implements HuggingFaceModel interface
	var _ HuggingFaceModel = (*MiniCPMConfig)(nil)
}
