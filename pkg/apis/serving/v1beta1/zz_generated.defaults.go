...
func SetObjectDefaults_ClusterTrainingRuntime(in *ClusterTrainingRuntime) {
	if in.Spec.ReplicaSpec != nil {
		for i := range in.Spec.ReplicaSpec.Template.Spec.Volumes {
			a := &in.Spec.ReplicaSpec.Template.Spec.Volumes[i]
			if a.VolumeSource.ISCSI != nil {
				if a.VolumeSource.ISCSI.ISCSIInterface == "" {
					a.VolumeSource.ISCSI.ISCSIInterface = "default"
				}
		...
		for i := range in.Spec.ReplicaSpec.Template.Spec.InitContainers {
			a := &in.Spec.ReplicaSpec.Template.Spec.InitContainers[i]
			for j := range a.Ports {
				b := &a.Ports[j]
				if b.Protocol == "" {
					b.Protocol = "TCP"
		...
		for i := range in.Spec.ReplicaSpec.Template.Spec.Containers {
			a := &in.Spec.ReplicaSpec.Template.Spec.Containers[i]
			for j := range a.Ports {
				b := &a.Ports[j]
				if b.Protocol == "" {
					b.Protocol = "TCP"
		...
		for i := range in.Spec.ReplicaSpec.Template.Spec.EphemeralContainers {
			a := &in.Spec.ReplicaSpec.Template.Spec.EphemeralContainers[i]
