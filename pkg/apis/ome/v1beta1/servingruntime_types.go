...
func (s *ServingRuntimeSpec) SupportsAcceleratorClass(acceleratorClass string) bool {
	if s.AcceleratorRequirements == nil || len(s.AcceleratorRequirements.AcceleratorClasses) == 0 {
		// No requirements means supports all accelerators
		return true
	}

	for _, supported := range s.AcceleratorRequirements.AcceleratorClasses {
