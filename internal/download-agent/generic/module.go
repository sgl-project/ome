...
var Module = fx.Provide(
	func(v *viper.Viper, e *env.Environment, params appParams) (*GenericDownloadAgent, error) {
		config, err := NewGenericConfig(
		...
		if err != nil {
			return nil, fmt.Errorf("error creating generic download agent config: %+v", err)
		}
		return NewGenericDownloadAgent(config)
	})
