...
func TestReplicaAgent_StartDeletesCompletionMarkerBeforeOverwritingTargetArtifact(t *testing.T) {
	agent, cleanup := newTestAgentForCompletionMarker(t)
	defer cleanup()
	agent.Config.TargetArtifactReuseAllowed = false
...
func TestReplicaAgent_PrepareTargetArtifactUploadLogsNeutralMessageWhenReuseDisabled(t *testing.T) {
	agent, cleanup := newTestAgentForCompletionMarker(t)
	defer cleanup()
	agent.Config.TargetArtifactReuseAllowed = false
	...
	mockLogger := agent.Logger.(*testingPkg.MockLogger)
	foundNeutralLog := false
	for _, call := range mockLogger.Calls {
		if call.Method != "Infof" {
			continue
		}
		format, ok := call.Arguments.Get(0).(string)
		if !ok {
			continue
		}
		assert.NotContains(t, strings.ToLower(format), "hf access validation marker")
		if format == "Target artifact reuse disabled; upload lock still applies" {
			foundNeutralLog = true
		}
	}
	assert.True(t, foundNeutralLog)
}
