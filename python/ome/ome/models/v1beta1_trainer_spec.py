...
class V1beta1TrainerSpec(BaseModel):
    ...
    num_proc_per_node: Optional[StrictStr] = Field(default=None, description="Number of processes/workers/slots on every training node. For the Torch runtime: `auto`, `cpu`, `gpu`, or int value can be set. For the MPI runtime only int value can be set.", alias="numProcPerNode")
