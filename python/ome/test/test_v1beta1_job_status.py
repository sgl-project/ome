# coding: utf-8

"""
OME

Python SDK for OME

The version of the OpenAPI document: v0.1
Generated by OpenAPI Generator (https://openapi-generator.tech)

Do not edit the class manually.
"""  # noqa: E501

import unittest

from ome.models.v1beta1_job_status import V1beta1JobStatus


class TestV1beta1JobStatus(unittest.TestCase):
    """V1beta1JobStatus unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1beta1JobStatus:
        """Test V1beta1JobStatus
        include_optional is a boolean, when False only required
        params are included, when True both required and
        optional params are included"""
        # uncomment below to create an instance of `V1beta1JobStatus`
        """
        model = V1beta1JobStatus()
        if include_optional:
            return V1beta1JobStatus(
                active = 56,
                failed = 56,
                name = '',
                ready = 56,
                succeeded = 56,
                suspended = 56
            )
        else:
            return V1beta1JobStatus(
                active = 56,
                failed = 56,
                name = '',
                ready = 56,
                succeeded = 56,
                suspended = 56,
        )
        """

    def testV1beta1JobStatus(self):
        """Test V1beta1JobStatus"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == "__main__":
    unittest.main()
